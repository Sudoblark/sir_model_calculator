...
def test_get_model_configuration():
    ...
    :return:
    """
    population, initial_infection, days, transmission_rate, recovery_rate, sir_model = _setup_basic_sir_model()
    sir_model_configuration = sir_model.get_model_configuration()
    assert sir_model_configuration[
               'closed_population'] == population, "Population returned in getModelConfiguration is not " \
...
def test_str_override():
    ...
    sir_model.run_simulation(callback_test)
    returned_string = str(sir_model).split("\n")
    for i in range(len(returned_string) - 1):
        # Matches floats or ints as data will alternate between these types throughout program's execution
        assert re.match('D [+-]?([0-9]*[.])?[0-9]+: S [+-]?([0-9]*[.])?[0-9]+: I [+-]?([0-9]*[.])?[0-9]+: R [+-]?([0-9]*[.])?[0-9]+',
                        returned_string[i]), "String output for day {0} is not in expected format".format(i)
